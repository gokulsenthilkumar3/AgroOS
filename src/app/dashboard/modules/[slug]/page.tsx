...
export default async function ModulePage({params}:{params:Promise<{slug:string}>}){
  ...
  return <>
    ...
    <section className="module-workspace-grid"><article className="panel"><div className="section-heading"><div><p className="eyebrow">CAPABILITIES</p><h2>Included workflows</h2></div></div><div className="capability-list">{module.features.map(feature=><div key={feature}><CheckCircle2 size={16}/><span><strong>{feature}</strong><small>Configured for this organisation</small></span><button aria-label={`Open ${feature}`}><ArrowUpRight size={15}/></button></div>)}</div></article><article className="panel action-panel"><p className="eyebrow">QUICK ACTIONS</p><h2>Move work forward</h2>{module.actions.map(action=><button className="workspace-action" key={action}><span>{action}</span><ArrowUpRight size={16}/></button>)}<div className="module-note"><strong>Shared context</strong><p>Actions inherit your active organisation, farm, role and audit policy.</p></div></article></section>
  </>;
}
