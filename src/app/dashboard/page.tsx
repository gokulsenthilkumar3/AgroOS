...
export default async function DashboardPage() {
  ...
  if (session.role === "CUSTOMER") redirect("/dashboard/marketplace");
  ...
  return (
    <>
      <header className="page-header">
        <div>
          <p className="eyebrow">{farm.location.toUpperCase()}</p>
          <h1>Good morning, {firstName}.</h1>
          <p className="muted">
            Here’s how {farm.name} and your wider portfolio are doing today.
          </p>
        </div>
        <button className="button secondary">Export report</button>
      </header>
      <section className="metric-grid">
        <Metric label="Growing zones" value="3" detail="All zones reporting" />
        <Metric
          label="Connected devices"
          value="2 / 3"
          detail="1 device needs attention"
          tone="warning"
        />
        <Metric
          label="Active alerts"
          value="2"
          detail="1 critical condition"
          tone="critical"
        />
        <Metric
          label="Next harvest"
      ...
      <section className="dashboard-grid">
        <article className="panel chart-panel">
          ...
          <div
            className="chart"
            role="img"
            aria-label="Temperature changed between 21 and 25 degrees Celsius over the last 24 hours"
          >
        ...
        <article className="panel device-panel">
          ...
          {devices.map((d) => (
            <div className="device-row" key={d.id}>
              <span
                className={`status-dot ${d.status === "Online" ? "healthy" : "warning"}`}
              />
              <div>
        ...
        <div className="crop-grid">
          {cropBatches.map((c) => (
            <article className="crop-card" key={c.crop}>
              ...
              <small>
                {c.progress}% complete · harvest {c.harvest}
              </small>
              <b
                className={
                  c.status === "On track" ? "healthy-text" : "warning-text"
                }
              >
                {c.status}
              </b>
