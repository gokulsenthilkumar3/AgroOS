...
export default async function AdminPage() {
  ...
  return <>
    <header className="page-header hero-header"><div><p className="eyebrow">PLATFORM CONTROL</p><h1>Administration</h1><p className="muted">Tenant operations, access governance, content moderation, integrations, and system health.</p></div><a className="button secondary" href="/api/v1/export?type=audit">Export audit log</a></header>
    ...
    <section className="admin-grid">{areas.map(([Icon,title,description]) => <article className="panel admin-card" key={title}><span className="admin-icon"><Icon size={19}/></span><h2>{title}</h2><p>{description}</p><a className="inline-action" href="#admin-records">Open workspace →</a></article>)}</section>
    ...
    <div id="admin-records"><ModuleRecords module="administration" name="Administration"/></div>
  </>;
}
